...
| Tether QVAC | Primary pending video evidence | Local QVAC runtime-backed invoice review when `QVAC_BASE_URL` points to localhost; otherwise QVAC-compatible deterministic fallback plus Local Invoice Agent decisioning. | Final video must show runtime or explicitly show fallback mode and the Local Invoice Agent panel. |
...
- RPC Fast: claimed only when `/api/status/rpc` proves `provider: "rpc-fast"` and `getHealth=ok`.
